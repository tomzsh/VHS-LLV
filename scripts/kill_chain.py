...
class KillChainBuilder:
    ...
    def _escalate_severity(self, severities: List[str],
        ...
        max_ind = max((order.get(s, 0) for s in severities), default=0)
        ...
        if pattern_severity == "critical":
            escalated = 4
        elif max_ind >= 3:  # At least one high
            escalated = 4
        elif len(severities) >= 2 and max_ind >= 2:
            escalated = 3
        elif len(severities) >= 2 and max_ind >= 1:
            escalated = 2
        else:
            escalated = max_ind
        # Invariant: a chain can never be rated below its strongest component.
        escalated = max(escalated, max_ind)
        return rank.get(escalated, "low")
